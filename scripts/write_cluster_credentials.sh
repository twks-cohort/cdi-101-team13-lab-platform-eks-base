...
cat kubeconfig_$CLUSTER | opw write CDI101Team13-platform-${CLUSTER} kubeconfig -
...
terraform output cluster_endpoint | tr -d \\n | sed 's/"//g' | opw write CDI101Team13-platform-${CLUSTER} cluster-endpoint -
terraform output cluster_certificate_authority_data | tr -d \\n | sed 's/"//g' | opw write CDI101Team13-platform-${CLUSTER} base64-certificate-authority-data -
