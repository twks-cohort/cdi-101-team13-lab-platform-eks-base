...
cat kubeconfig_$CLUSTER | opw write blue-platform-${CLUSTER} kubeconfig -

# terraform-aws-eks module v18 method
# terraform output kubeconfig | grep -v "EOT" | opw write blue-platform-${CLUSTER} kubeconfig -

# write cluster url and pubic certificate to 1password
terraform output cluster_endpoint | tr -d \\n | sed 's/"//g' | opw write blue-platform-${CLUSTER} cluster-endpoint -
terraform output cluster_certificate_authority_data | tr -d \\n | sed 's/"//g' | opw write blue-platform-${CLUSTER} base64-certificate-authority-data -
